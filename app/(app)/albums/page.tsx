...
export default function AlbumsPage() {
    ...
    const visibleAlbums =
        albums?.slice((albumPage - 1) * ALBUMS_PER_PAGE, albumPage * ALBUMS_PER_PAGE) ?? []
    ...
    return (
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
            ...
            {/* Inline create form */}
            {showCreate && (
                <div className="border-border bg-card mb-6 flex items-center gap-3 rounded-xl border p-4">
            ...
            {!isLoading && albums?.length === 0 && (
                <div className="flex flex-col items-center justify-center py-24 text-center">
                    <div className="border-border bg-card mb-5 rounded-2xl border p-6">
                        <FolderOpen className="text-muted-foreground/40 size-10" />
                    </div>
...
function AlbumCard({ album }: { album: Album }) {
    ...
    return (
        <div
            data-card
            className="group border-border bg-card relative flex flex-col overflow-hidden rounded-xl border transition-shadow hover:shadow-md"
        >
            <Link href={`/albums/${album.id}`} className="block">
                <div className="bg-muted relative aspect-square overflow-hidden">
                    <AlbumCoverGrid
                        thumbs={thumbs.map((i) => ({ url: displaySrc(i), filename: i.filename }))}
                        name={album.name}
                    />
                </div>
                <div className="p-3">
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                <span className="font-medium">{album.name}</span> will be deleted.
...
function AlbumCoverGrid({
    thumbs,
}: {
    thumbs: { url: string | null; filename: string }[]
    name: string
}) {
    if (thumbs.length === 0) {
        return (
            <div className="flex h-full items-center justify-center">
                <FolderOpen className="text-muted-foreground/25 size-10" />
            </div>
        )
    }
    ...
    return (
        <div className="bg-border grid h-full w-full grid-cols-2 grid-rows-2 gap-px">
            {[0, 1, 2, 3].map((i) => (
