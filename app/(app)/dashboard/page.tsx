...
export default function DashboardPage() {
    ...
    const isEmpty =
        !galleryLoading && !albumsLoading && !favsLoading && !hasGallery && !hasAlbums && !hasFavs
            ...
            {(favsLoading || hasFavs) && (
                ...
                >
                    {favsLoading
                        ...
                        : favourites!.items.map((img) => <FavStrip key={img.id} image={img} />)}
                </Section>
            )}
        ...
        >
            {src ? (
                <VaultImage
                    ...
                    className={cn(
                        "object-cover transition-[opacity,transform,scale] duration-300",
...
function AlbumCard({
    ...
    album: { id: string; name: string; image_count: number; cover_url?: string }
}) {
    const [loaded, setLoaded] = useState(false)
    return (
        <Link
...
function EmptyState() {
    ...
    return (
        <div ref={ref} className="flex flex-col items-center justify-center py-28 text-center">
            <div className="border-border bg-card mb-5 rounded-2xl border p-7 shadow-sm">
                <Images className="text-muted-foreground/40 size-12" />
            </div>
            <h2 className="text-xl font-semibold">Welcome to vault</h2>
            <p className="text-muted-foreground mt-2 max-w-sm text-sm">
                Upload your first photo to get started. Your gallery, albums, and favourites will
                appear here.
            </p>
