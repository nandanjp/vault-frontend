        ...
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
            ...
            {!isLoading && stories?.length === 0 && (
                <div className="flex flex-col items-center justify-center py-28 text-center">
                    <div className="border-border bg-card mb-5 rounded-2xl border p-7">
                        <Clapperboard className="text-muted-foreground/40 size-10" />
                    </div>
                    <h2 className="text-lg font-semibold">No stories yet</h2>
                    <p className="text-muted-foreground mt-2 max-w-sm text-sm">
                        Create your first story — pick images, set timings, and build an
...
function StoryCard({ story }: { story: Story }) {
    ...
    return (
        <div
            data-card
            className="group border-border bg-card relative flex min-w-0 flex-col overflow-hidden rounded-xl border transition-shadow hover:shadow-md"
        >
                ...
                <div className="bg-muted relative aspect-[9/16] overflow-hidden">
                    {story.cover_url ? (
                        ...
                        <div className="flex h-full items-center justify-center">
                            <Clapperboard className="text-muted-foreground/25 size-8" />
                        </div>
                    )}
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                <span className="font-medium">{story.title}</span> will be
                                permanently deleted.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
