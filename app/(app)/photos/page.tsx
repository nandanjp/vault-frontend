...
export default function PhotosPage() {
    ...
    const pendingCount =
        data?.items.filter((i) => i.status === "pending" || i.status === "processing").length ?? 0
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
                        ...
                        {data.items.map((image) => (
                            ...
                            >
                                <ImageCard
                                    ...
                                    isDeleting={
                                        deleteMedia.isPending && deleteMedia.variables === image.id
                                    }
                                />
                                {image.status === "ready" && (
                                    <button
                                        ...
                                        title="Add to album"
                                        className="absolute top-2 left-2 z-10 hidden size-7 items-center justify-center rounded-full bg-black/50 text-white backdrop-blur-sm transition-colors group-hover/wrap:flex hover:bg-black/75"
                                    >
            ...
            {addToAlbumTarget && (
                <AlbumPickerDialog
                    open
                    onOpenChange={(open) => {
                        if (!open) setAddToAlbumTarget(null)
                    }}
