...
export const authApi = {
    register: (email: string, password: string) =>
        request<{ id: string; email: string }>(`${AUTH_URL}/register`, {
            method: "POST",
            body: JSON.stringify({ email, password }),
        }),

    login: (email: string, password: string) =>
        request<TokenPair>(`${AUTH_URL}/login`, {
            method: "POST",
            body: JSON.stringify({ email, password }),
        }),

    refresh: (refresh_token: string) =>
        request<TokenPair>(`${AUTH_URL}/refresh`, {
            method: "POST",
...
export const backendApi = {
    ...
    listMedia: (token: string, page = 1, limit = 20) =>
        request<ImagePage>(`${BACKEND_URL}/api/media?page=${page}&limit=${limit}`, {}, token),

    getMedia: (token: string, id: string) =>
        request<Image & { url?: string }>(`${BACKEND_URL}/api/media/${id}`, {}, token),

    deleteMedia: (token: string, id: string) =>
        request<void>(`${BACKEND_URL}/api/media/${id}`, { method: "DELETE" }, token),
    ...
    deleteAlbum: (token: string, id: string) =>
        request<void>(`${BACKEND_URL}/api/albums/${id}`, { method: "DELETE" }, token),

    listAlbumImages: (token: string, id: string, page = 1, limit = 20) =>
        request<ImagePage>(
            `${BACKEND_URL}/api/albums/${id}/images?page=${page}&limit=${limit}`,
            {},
            token
        ),
    ...
    updateStory: (
        ...
        request<StoryDetail>(
            ...
            {
                method: "PUT",
                body: JSON.stringify({ title, slides, spotify_track_id: spotifyTrackId ?? null }),
            },
            token
        ),
