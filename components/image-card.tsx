...
export function ImageCard({
    ...
    const imageArea = (
        <div
            className={cn("bg-muted relative w-full overflow-hidden", !natural && "aspect-square")}
            style={aspectStyle}
        >
            {isReady ? (
                src ? (
                    <>
                        <VaultImage
                            ...
                            className={cn(
                                "h-full w-full object-cover transition-[opacity,transform,scale] duration-300",
                                imgLoaded && "group-hover:scale-105"
                            )}
                            loading="lazy"
                        />
                        {/* Filename overlay — slides up on hover */}
                        <div className="absolute inset-x-0 bottom-0 translate-y-full bg-gradient-to-t from-black/75 to-transparent px-3 pt-10 pb-2.5 transition-[transform,translate] duration-300 ease-out group-hover:translate-y-0">
                    ...
                    <div className="text-muted-foreground flex h-full min-h-[140px] flex-col items-center justify-center gap-2">
            ...
            {isProcessing && <div className="bg-background/10 absolute inset-0" />}
        </div>
    )

    return (
        <div className="group border-border bg-card relative overflow-hidden rounded-xl border transition-all hover:shadow-md">
            {isReady ? (
                onView ? (
