...
const navItemClass = (active: boolean, collapsed: boolean) =>
    cn(
        ...
        !collapsed &&
            active &&
            "border-l-2 border-primary bg-primary/[.07] pl-2.5 pr-3 text-foreground",
        ...
        collapsed &&
            !active &&
            "justify-center px-2 text-muted-foreground hover:bg-muted hover:text-foreground"
    )
...
export function AppSidebar() {
    ...
    const BottomActions = (
        <div className="border-border space-y-0.5 border-t p-2">
            <Link href="/upload" onClick={closeMobile} className={navItemClass(false, collapsed)}>
    ...
    return (
        <>
            ...
            <aside
                className={cn(
                    "border-border bg-card relative hidden h-screen flex-col border-r transition-all duration-200 lg:flex",
                    w
                )}
            >
                ...
                <button
                    ...
                    className="border-border bg-background text-muted-foreground hover:text-foreground absolute top-[60px] -right-3 z-10 flex size-6 items-center justify-center rounded-full border shadow-sm transition-colors"
                >
                    {collapsed ? (
            ...
            <aside
                className={cn(
                    "border-border bg-card fixed top-0 left-0 z-50 flex h-full w-56 flex-col border-r transition-transform duration-200 lg:hidden",
...
function SidebarHeader({ collapsed }: { collapsed: boolean }) {
    return (
        <div className="border-border flex h-14 shrink-0 items-center border-b px-4">
            ...
            >
                <Vault className="text-primary size-4 shrink-0" />
...
export function MobileTopBar() {
    ...
    return (
        <div className="border-border bg-background/90 sticky top-0 z-30 flex h-14 items-center gap-3 border-b px-4 backdrop-blur lg:hidden">
            <button
                onClick={openMobile}
