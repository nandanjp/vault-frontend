...
export function AlbumPickerDialog({ open, onOpenChange, imageId }: AlbumPickerDialogProps) {
    ...
    const imagePreview = image ? (
        <div className="border-border/50 bg-muted relative size-12 shrink-0 overflow-hidden rounded-lg border shadow-sm">
            {imgSrc ? (
                <VaultImage src={imgSrc} alt={image.filename} fill className="object-cover" />
            ) : (
                <div className="flex h-full items-center justify-center">
                    <ImageIcon className="text-muted-foreground/30 size-5" />
                </div>
            )}
        </div>
    ) : null
                ...
                <div className="space-y-0.5">
                    {filtered.map((album) => (
                        <button
                            ...
                            className="hover:bg-muted flex w-full items-center gap-3.5 rounded-xl px-3 py-2.5 text-left transition-colors disabled:opacity-50"
                        >
                            <div className="bg-muted relative size-11 shrink-0 overflow-hidden rounded-lg">
                                {album.cover_url ? (
                                    ...
                                    <div className="flex h-full items-center justify-center">
                                        <FolderOpen className="text-muted-foreground/40 size-5" />
                                    </div>
                                )}
                            </div>
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-muted-foreground mt-0.5 text-xs">
                                    {album.image_count}{" "}
                                    {album.image_count === 1 ? "photo" : "photos"}
                                </p>
                            </div>
                            <Check className="text-muted-foreground/30 size-4 shrink-0" />
                        </button>
                    ))}
                </div>
            )}
