...
export function AlbumSlideshow({ images, initialIndex = 0, onClose }: AlbumSlideshowProps) {
    ...
    useEffect(() => {
        const handler = (e: KeyboardEvent) => {
            if (e.key === "ArrowLeft")
                goTo((currentRef.current - 1 + images.length) % images.length)
        ...
        >
            ...
            >
                ...
                <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between px-4 pt-4">
                    <span className="pointer-events-auto text-sm text-white/50 tabular-nums select-none">
                    ...
                    <div className="pointer-events-auto flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="hidden size-8 items-center justify-center rounded-full bg-black/40 text-white backdrop-blur-sm transition-colors hover:bg-black/60 sm:flex"
                        >
                            {expanded ? (
                ...
                <div className="shrink-0 border-t border-white/10 bg-black/80 px-3 pt-4 pb-3">
                    <div
                        ref={thumbStripRef}
                        className="flex scrollbar-none gap-2 overflow-x-auto px-0.5 py-1"
                    >
...
function SlideshowCarousel({ images, current, onGoTo }: SlideshowCarouselProps) {
    ...
    useEffect(() => {
        ...
        if (toEl)
            gsap.fromTo(toEl, { opacity: 0 }, { opacity: 1, duration: 0.45, ease: "power2.inOut" })
        prevRef.current = current
    }, [current])
    ...
    return (
        <div className="relative h-full w-full overflow-hidden bg-black">
            ...
            {images.length > 1 && (
                <>
                    <button
                        ...
                        className="absolute top-1/2 left-3 z-10 flex size-10 -translate-y-1/2 items-center justify-center rounded-full bg-black/30 text-white backdrop-blur-sm transition-colors hover:bg-black/50"
                    >
                        <ChevronLeft className="size-5" />
                    </button>
                    <button
                        onClick={() => onGoTo((current + 1) % images.length)}
                        className="absolute top-1/2 right-3 z-10 flex size-10 -translate-y-1/2 items-center justify-center rounded-full bg-black/30 text-white backdrop-blur-sm transition-colors hover:bg-black/50"
                    >
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-linear-to-t from-black/70 via-black/20 to-transparent px-6 pt-16 pb-4">
